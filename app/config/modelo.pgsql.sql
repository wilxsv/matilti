...
-- Started on 2013-11-07 15:09:09 CST
...
--
-- TOC entry 572 (class 1247 OID 16386)
-- Dependencies: 573 5
...
--
-- TOC entry 574 (class 1247 OID 16388)
-- Dependencies: 575 5
...
--
-- TOC entry 576 (class 1247 OID 16390)
-- Dependencies: 577 5
...
--
-- TOC entry 221 (class 1255 OID 16392)
-- Dependencies: 695 5
...
--
-- TOC entry 222 (class 1255 OID 16393)
-- Dependencies: 695 5
-- Name: anda(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION anda() RETURNS integer
    LANGUAGE plpgsql
    AS $$
/**
 * Funcion que registra una denuncia dirigida a ANDA
...
--
-- TOC entry 223 (class 1255 OID 16394)
-- Dependencies: 5 695
-- Name: anda(text, text, timestamp without time zone, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION anda(text, text, timestamp without time zone, text) RETURNS integer
    LANGUAGE plpgsql
    AS $_$
/**
 * Funcion que registra una denuncia dirigida a ANDA
...
DECLARE
    ...
    v_resultado INTEGER;
BEGIN
   RAISE INFO ' :: ENVIADO A DENUNCIA() :: ';
   v_resultado := denuncia(numero, mensaje, hora, "anda");
END;
$_$;
	...
	IF v_item > 0  THEN
	    IF usuario_es_valido(numero,"cerrar") = TRUE THEN
			...
			IF v_id_denuncia > 0 THEN
				SELECT * INTO v_dato FROM "scd_denuncia" WHERE ("id" = v_id_denuncia);
...
-- Dependencies: 695 5
-- Name: defensoria(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION defensoria() RETURNS integer
    LANGUAGE plpgsql
    AS $$
/**
 * Funcion que registra una denuncia dirigida a defensoria
 * se recibe un numero y una cadena, si el telefono existe se registra,
 * si no, se envia a otros_sms y se le notifica al remitente.
 * 
 * Acceso: publico
...
-- Dependencies: 5 695
-- Name: defensoria(text, text, timestamp without time zone, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION defensoria(text, text, timestamp without time zone, text) RETURNS integer
    LANGUAGE plpgsql
    AS $_$
/**
 * Funcion que registra una denuncia dirigida a defensoria
...
BEGIN
   RAISE INFO ' :: ENVIADO A DENUNCIA() :: ';
   v_resultado := denuncia(numero, mensaje, hora, "defensoria");
END;
$_$;
	    ...
	    IF usuario_es_valido(numero, parametro) = TRUE THEN
		    ...
		    ELSE
		        ...
		        IF (v_entidad.telefonoentidad NOTNULL) THEN
		            v_id := enviar_sms(get_numero(v_entidad.telefonoentidad), mensaje);
		        END IF;
		        RETURN 1;
		    END IF;
	    ELSE
...
CREATE FUNCTION envia_sms() RETURNS trigger
    ...
    AS $$
/**
 * Funcion que envia el mensaje haciendo uso de enviar_sms,
 * Acceso: publico
 * Autor:  Gualberto Escalante - gual23@gmail.com
 * Fecha: 2012.05.15
 *
...
--
-- TOC entry 225 (class 1255 OID 16402)
-- Dependencies: 5 695
...
--
-- TOC entry 232 (class 1255 OID 16403)
-- Dependencies: 695 5
...
BEGIN
    ...
    IF v_numero > 0 THEN
        ...
        IF (v_item.id NOTNULL) THEN
            v_msg := 'Hola '||v_item.nombreusuario||' '||v_item.apellidousuario||' tu estado es ['||v_item.nombreestado||'] y usas la red como ['||v_item.nombrerol||']';
    ...
    IF (TG_OP = 'INSERT') THEN
        ...
        IF (v_item.id NOTNULL) THEN
            ...
            ELSIF (v_parametro = 'masivo' OR v_parametro = 'todas') THEN --envio de mensaje masio a usuarios
                v_id := masivo(NEW.sendernumber, NEW.textdecoded);
            ...
            ELSIF (v_parametro = 'panico' OR v_parametro = 'a') THEN --Envia una notificacion a la policia y solicita una patrulla
                v_id := salir(NEW.sendernumber);
...
--
-- TOC entry 239 (class 1255 OID 16409)
-- Dependencies: 695 5
...
--
-- TOC entry 240 (class 1255 OID 16410)
-- Dependencies: 695 5
...
--
-- TOC entry 241 (class 1255 OID 16411)
-- Dependencies: 695 5
...
-- Dependencies: 695 5
-- Name: masivo(text, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION masivo(text, text) RETURNS integer
    LANGUAGE plpgsql
    AS $_$
/**
 * Funcion que envia un mensaje a todos los usuarios del sistema
...
DECLARE
    ...
    v_masivo TEXT;
BEGIN
    IF usuario_es_valido(numero, 'masivo') THEN
        v_total := 0;
        v_masivo := substring(trim(v_msg::text) from 7 for char_length(trim(v_msg::text)));
        SELECT id INTO v_item FROM scd_regla_sms WHERE prefijoregla = 'masivo';
        INSERT INTO scd_recibido(mensajerecibido, usuario_id, fechahorarecibido, regla_id)
        VALUES (v_msg, get_id_usuario(get_numero(numero)), now()::timestamp(0) without time zone, v_item.id);
        IF (character_length(v_masivo) > 15 ) THEN
...
--
-- TOC entry 246 (class 1255 OID 16417)
-- Dependencies: 695 5
...
--
-- TOC entry 247 (class 1255 OID 16418)
-- Dependencies: 695 5
...
--
-- TOC entry 253 (class 1255 OID 16424)
-- Dependencies: 5 695
...
--
-- TOC entry 254 (class 1255 OID 16425)
-- Dependencies: 695 5
...
--
-- TOC entry 255 (class 1255 OID 16426)
-- Dependencies: 695 5
...
--
-- TOC entry 256 (class 1255 OID 16427)
-- Dependencies: 5 695
...
--
-- TOC entry 257 (class 1255 OID 16428)
-- Dependencies: 695 5
...
--
-- TOC entry 258 (class 1255 OID 16429)
-- Dependencies: 695 5
...
--
-- TOC entry 259 (class 1255 OID 16430)
-- Dependencies: 5 695
...
--
-- TOC entry 260 (class 1255 OID 16431)
-- Dependencies: 695 5
...
--
-- TOC entry 261 (class 1255 OID 16432)
-- Dependencies: 695 5
...
--
-- TOC entry 262 (class 1255 OID 16433)
-- Dependencies: 5 695
...
-- Dependencies: 5
-- Name: daemons; Type: TABLE; Schema: public; Owner: -; Tablespace: 
--
...
--
-- TOC entry 162 (class 1259 OID 16440)
-- Dependencies: 5 161
...
--
-- TOC entry 163 (class 1259 OID 16442)
-- Dependencies: 2060 5
...
--
-- TOC entry 164 (class 1259 OID 16446)
-- Dependencies: 163 5
...
--
-- TOC entry 165 (class 1259 OID 16448)
-- Dependencies: 2062 2063 2064 2065 2066 2067 2068 2069 2071 5
...
--
-- TOC entry 166 (class 1259 OID 16463)
-- Dependencies: 165 5
...
--
-- TOC entry 167 (class 1259 OID 16465)
-- Dependencies: 2072 2073 2074 2075 2076 2077 2078 2079 2080 2081 2082 2084 2085 5
-- Name: outbox; Type: TABLE; Schema: public; Owner: -; Tablespace: 
--
...
--
-- TOC entry 168 (class 1259 OID 16484)
-- Dependencies: 167 5
...
--
-- TOC entry 169 (class 1259 OID 16486)
-- Dependencies: 2086 2087 2088 2090 5
-- Name: outbox_multipart; Type: TABLE; Schema: public; Owner: -; Tablespace: 
--
...
--
-- TOC entry 171 (class 1259 OID 16498)
-- Dependencies: 2091 5
-- Name: pbk; Type: TABLE; Schema: public; Owner: -; Tablespace: 
--
...
-- Dependencies: 5
-- Name: pbk_groups; Type: TABLE; Schema: public; Owner: -; Tablespace: 
--
...
-- Dependencies: 2094 2095 2096 2097 2098 2099 2100 2101 2102 5
...
--
-- TOC entry 176 (class 1259 OID 16530)
-- Dependencies: 5
-- Name: scd_accion; Type: TABLE; Schema: public; Owner: -; Tablespace: 
--
...
-- Dependencies: 2104 5
-- Name: scd_denuncia; Type: TABLE; Schema: public; Owner: -; Tablespace: 
--
...
-- Dependencies: 2106 5
-- Name: scd_encuesta; Type: TABLE; Schema: public; Owner: -; Tablespace: 
--
...
-- Dependencies: 572 5 576
-- Name: scd_entidad; Type: TABLE; Schema: public; Owner: -; Tablespace: 
--
...
--
-- TOC entry 184 (class 1259 OID 16561)
-- Dependencies: 5
-- Name: scd_enviado; Type: TABLE; Schema: public; Owner: -; Tablespace: 
--
...
--
-- TOC entry 186 (class 1259 OID 16566)
-- Dependencies: 5
-- Name: scd_estado; Type: TABLE; Schema: public; Owner: -; Tablespace: 
--
...
--
-- TOC entry 187 (class 1259 OID 16572)
-- Dependencies: 186 5
...
-- Dependencies: 2111 574 5
-- Name: scd_historial_operacion; Type: TABLE; Schema: public; Owner: -; Tablespace: 
--
...
--
-- TOC entry 189 (class 1259 OID 16581)
-- Dependencies: 5 188
...
-- Dependencies: 5
-- Name: scd_historial_permiso; Type: TABLE; Schema: public; Owner: -; Tablespace: 
--
...
-- Dependencies: 5
-- Name: scd_localidad; Type: TABLE; Schema: public; Owner: -; Tablespace: 
--
...
--
-- TOC entry 194 (class 1259 OID 16596)
-- Dependencies: 2115 5
-- Name: scd_otros_sms; Type: TABLE; Schema: public; Owner: -; Tablespace: 
--
...
--
-- TOC entry 195 (class 1259 OID 16603)
-- Dependencies: 5 194
...
-- Dependencies: 5
-- Name: scd_recibido; Type: TABLE; Schema: public; Owner: -; Tablespace: 
--
...
-- Dependencies: 5
-- Name: scd_regla_rol; Type: TABLE; Schema: public; Owner: -; Tablespace: 
--
...
--
-- TOC entry 199 (class 1259 OID 16613)
-- Dependencies: 2118 5
-- Name: scd_regla_sms; Type: TABLE; Schema: public; Owner: -; Tablespace: 
--
...
--
-- TOC entry 201 (class 1259 OID 16619)
-- Dependencies: 5
-- Name: scd_rol; Type: TABLE; Schema: public; Owner: -; Tablespace: 
--
...
-- Dependencies: 2121 2122 2123 576 574 572 5
-- Name: scd_usuario; Type: TABLE; Schema: public; Owner: -; Tablespace: 
--
...
-- Dependencies: 5
-- Name: scd_usuario_entidad; Type: TABLE; Schema: public; Owner: -; Tablespace: 
--
...
-- Dependencies: 5
-- Name: scd_usuario_rol; Type: TABLE; Schema: public; Owner: -; Tablespace: 
--
...
-- Dependencies: 2125 2126 2127 2128 2129 2130 2131 2132 2133 2134 2135 2136 2137 2139 2140 5
-- Name: sentitems; Type: TABLE; Schema: public; Owner: -; Tablespace: 
--
...
--
-- TOC entry 2059 (class 2604 OID 16667)
...
--
-- TOC entry 2061 (class 2604 OID 16668)
...
--
-- TOC entry 2070 (class 2604 OID 16669)
...
--
-- TOC entry 2083 (class 2604 OID 16670)
...
--
-- TOC entry 2089 (class 2604 OID 16671)
...
--
-- TOC entry 2092 (class 2604 OID 16672)
...
--
-- TOC entry 2093 (class 2604 OID 16673)
...
--
-- TOC entry 2103 (class 2604 OID 16674)
...
--
-- TOC entry 2105 (class 2604 OID 16675)
...
--
-- TOC entry 2107 (class 2604 OID 16676)
...
--
-- TOC entry 2108 (class 2604 OID 16677)
...
--
-- TOC entry 2109 (class 2604 OID 16678)
...
--
-- TOC entry 2110 (class 2604 OID 16679)
...
--
-- TOC entry 2112 (class 2604 OID 16680)
...
--
-- TOC entry 2113 (class 2604 OID 16681)
...
--
-- TOC entry 2114 (class 2604 OID 16682)
...
--
-- TOC entry 2116 (class 2604 OID 16683)
...
--
-- TOC entry 2117 (class 2604 OID 16684)
...
--
-- TOC entry 2119 (class 2604 OID 16685)
...
--
-- TOC entry 2120 (class 2604 OID 16686)
...
--
-- TOC entry 2124 (class 2604 OID 16687)
...
--
-- TOC entry 2138 (class 2604 OID 16688)
...
--
-- TOC entry 2146 (class 2606 OID 16690)
...
--
-- TOC entry 2152 (class 2606 OID 16692)
-- Dependencies: 169 169 169 2249
-- Name: outbox_multipart_pkey; Type: CONSTRAINT; Schema: public; Owner: -; Tablespace: 
--
...
--
-- TOC entry 2149 (class 2606 OID 16694)
-- Dependencies: 167 167 2249
-- Name: outbox_pkey; Type: CONSTRAINT; Schema: public; Owner: -; Tablespace: 
--
...
--
-- TOC entry 2156 (class 2606 OID 16696)
-- Dependencies: 172 172 2249
-- Name: pbk_groups_pkey; Type: CONSTRAINT; Schema: public; Owner: -; Tablespace: 
--
...
--
-- TOC entry 2154 (class 2606 OID 16698)
-- Dependencies: 171 171 2249
-- Name: pbk_pkey; Type: CONSTRAINT; Schema: public; Owner: -; Tablespace: 
--
...
--
-- TOC entry 2158 (class 2606 OID 16700)
-- Dependencies: 175 175 2249
...
--
-- TOC entry 2142 (class 2606 OID 16702)
-- Dependencies: 161 161 2249
-- Name: pk_deamons; Type: CONSTRAINT; Schema: public; Owner: -; Tablespace: 
--
...
--
-- TOC entry 2174 (class 2606 OID 16704)
-- Dependencies: 186 186 2249
...
--
-- TOC entry 2144 (class 2606 OID 16706)
-- Dependencies: 163 163 2249
-- Name: pk_gammu; Type: CONSTRAINT; Schema: public; Owner: -; Tablespace: 
--
...
--
-- TOC entry 2182 (class 2606 OID 16708)
-- Dependencies: 192 192 2249
...
--
-- TOC entry 2184 (class 2606 OID 16710)
-- Dependencies: 194 194 2249
-- Name: pk_otros_sms; Type: CONSTRAINT; Schema: public; Owner: -; Tablespace: 
--
...
--
-- TOC entry 2186 (class 2606 OID 16712)
-- Dependencies: 196 196 2249
-- Name: pk_recibido; Type: CONSTRAINT; Schema: public; Owner: -; Tablespace: 
--
...
--
-- TOC entry 2190 (class 2606 OID 16714)
-- Dependencies: 199 199 2249
-- Name: pk_regla; Type: CONSTRAINT; Schema: public; Owner: -; Tablespace: 
--
...
--
-- TOC entry 2188 (class 2606 OID 16716)
-- Dependencies: 198 198 198 2249
-- Name: pk_regla_rol; Type: CONSTRAINT; Schema: public; Owner: -; Tablespace: 
--
...
--
-- TOC entry 2196 (class 2606 OID 16718)
-- Dependencies: 201 201 2249
...
--
-- TOC entry 2160 (class 2606 OID 16720)
-- Dependencies: 176 176 2249
-- Name: pk_scd_accion; Type: CONSTRAINT; Schema: public; Owner: -; Tablespace: 
--
...
--
-- TOC entry 2178 (class 2606 OID 16722)
-- Dependencies: 188 188 2249
-- Name: pk_scd_bitacora; Type: CONSTRAINT; Schema: public; Owner: -; Tablespace: 
--
...
--
-- TOC entry 2180 (class 2606 OID 16724)
-- Dependencies: 190 190 2249
-- Name: pk_scd_historial_permiso; Type: CONSTRAINT; Schema: public; Owner: -; Tablespace: 
--
...
--
-- TOC entry 2200 (class 2606 OID 16726)
-- Dependencies: 203 203 2249
...
--
-- TOC entry 2206 (class 2606 OID 16728)
-- Dependencies: 204 204 204 2249
-- Name: pk_usuario_entidad; Type: CONSTRAINT; Schema: public; Owner: -; Tablespace: 
--
...
--
-- TOC entry 2163 (class 2606 OID 16732)
-- Dependencies: 178 178 2249
-- Name: scd_denuncia_pkey; Type: CONSTRAINT; Schema: public; Owner: -; Tablespace: 
--
...
--
-- TOC entry 2165 (class 2606 OID 16734)
-- Dependencies: 180 180 2249
-- Name: scd_encuesta_pkey; Type: CONSTRAINT; Schema: public; Owner: -; Tablespace: 
--
...
--
-- TOC entry 2167 (class 2606 OID 16736)
-- Dependencies: 182 182 2249
-- Name: scd_entidad_pkey; Type: CONSTRAINT; Schema: public; Owner: -; Tablespace: 
--
...
--
-- TOC entry 2172 (class 2606 OID 16738)
-- Dependencies: 184 184 2249
-- Name: scd_enviado_pkey; Type: CONSTRAINT; Schema: public; Owner: -; Tablespace: 
--
...
--
-- TOC entry 2176 (class 2606 OID 16740)
-- Dependencies: 186 186 2249
-- Name: scd_estado_nombreestado_key; Type: CONSTRAINT; Schema: public; Owner: -; Tablespace: 
--
...
--
-- TOC entry 2198 (class 2606 OID 16742)
-- Dependencies: 201 201 2249
...
--
-- TOC entry 2212 (class 2606 OID 16744)
-- Dependencies: 207 207 207 2249
-- Name: sentitems_pkey; Type: CONSTRAINT; Schema: public; Owner: -; Tablespace: 
--
...
--
-- TOC entry 2202 (class 2606 OID 16746)
-- Dependencies: 203 203 2249
...
--
-- TOC entry 2169 (class 2606 OID 16748)
-- Dependencies: 182 182 2249
-- Name: unique_entidad; Type: CONSTRAINT; Schema: public; Owner: -; Tablespace: 
--
...
--
-- TOC entry 2204 (class 2606 OID 16750)
-- Dependencies: 203 203 2249
...
--
-- TOC entry 2192 (class 2606 OID 16752)
-- Dependencies: 199 199 2249
-- Name: unique_nombre_regla; Type: CONSTRAINT; Schema: public; Owner: -; Tablespace: 
--
...
--
-- TOC entry 2194 (class 2606 OID 16754)
-- Dependencies: 199 199 2249
-- Name: unique_patron_regla; Type: CONSTRAINT; Schema: public; Owner: -; Tablespace: 
--
...
--
-- TOC entry 2161 (class 1259 OID 16755)
-- Dependencies: 178 2249
-- Name: fki_parent_denuncia; Type: INDEX; Schema: public; Owner: -; Tablespace: 
--
...
--
-- TOC entry 2170 (class 1259 OID 16756)
-- Dependencies: 184 2249
-- Name: fki_scd_enviado_encuesta_id_fkey -> scd_encuesta; Type: INDEX; Schema: public; Owner: -; Tablespace: 
--
...
--
-- TOC entry 2147 (class 1259 OID 16757)
-- Dependencies: 167 167 2249
-- Name: outbox_date; Type: INDEX; Schema: public; Owner: -; Tablespace: 
--
...
--
-- TOC entry 2150 (class 1259 OID 16758)
-- Dependencies: 167 2249
-- Name: outbox_sender; Type: INDEX; Schema: public; Owner: -; Tablespace: 
--
...
--
-- TOC entry 2209 (class 1259 OID 16759)
-- Dependencies: 207 2249
-- Name: sentitems_date; Type: INDEX; Schema: public; Owner: -; Tablespace: 
--
...
--
-- TOC entry 2210 (class 1259 OID 16760)
-- Dependencies: 207 2249
-- Name: sentitems_dest; Type: INDEX; Schema: public; Owner: -; Tablespace: 
--
...
--
-- TOC entry 2213 (class 1259 OID 16761)
-- Dependencies: 207 2249
-- Name: sentitems_sender; Type: INDEX; Schema: public; Owner: -; Tablespace: 
--
...
--
-- TOC entry 2214 (class 1259 OID 16762)
-- Dependencies: 207 2249
-- Name: sentitems_tpmr; Type: INDEX; Schema: public; Owner: -; Tablespace: 
--
...
--
-- TOC entry 2241 (class 2620 OID 16763)
-- Dependencies: 184 224 2249
...
--
-- TOC entry 2237 (class 2620 OID 16764)
-- Dependencies: 165 233 2249
...
--
-- TOC entry 2242 (class 2620 OID 16765)
-- Dependencies: 186 240 2249
...
--
-- TOC entry 2244 (class 2620 OID 16766)
-- Dependencies: 201 241 2249
...
--
-- TOC entry 2243 (class 2620 OID 16767)
-- Dependencies: 246 199 2249
...
--
-- TOC entry 2245 (class 2620 OID 16768)
-- Dependencies: 247 203 2249
...
--
-- TOC entry 2246 (class 2620 OID 16769)
-- Dependencies: 258 203 2249
...
--
-- TOC entry 2238 (class 2620 OID 16770)
-- Dependencies: 260 165 2249
...
--
-- TOC entry 2239 (class 2620 OID 16771)
-- Dependencies: 260 167 2249
-- Name: update_timestamp; Type: TRIGGER; Schema: public; Owner: -
--
...
--
-- TOC entry 2240 (class 2620 OID 16772)
-- Dependencies: 175 260 2249
-- Name: update_timestamp; Type: TRIGGER; Schema: public; Owner: -
--
...
--
-- TOC entry 2247 (class 2620 OID 16773)
-- Dependencies: 207 260 2249
-- Name: update_timestamp; Type: TRIGGER; Schema: public; Owner: -
--
...
--
-- TOC entry 2233 (class 2606 OID 16774)
-- Dependencies: 204 182 2166 2249
...
--
-- TOC entry 2231 (class 2606 OID 16779)
-- Dependencies: 186 2173 203 2249
...
--
-- TOC entry 2226 (class 2606 OID 16784)
-- Dependencies: 192 192 2181 2249
...
--
-- TOC entry 2232 (class 2606 OID 16789)
-- Dependencies: 2181 203 192 2249
...
--
-- TOC entry 2229 (class 2606 OID 16794)
-- Dependencies: 199 198 2189 2249
...
--
-- TOC entry 2235 (class 2606 OID 16799)
-- Dependencies: 201 2195 206 2249
...
--
-- TOC entry 2230 (class 2606 OID 16804)
-- Dependencies: 2195 198 201 2249
-- Name: fk_rol; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- TOC entry 2215 (class 2606 OID 16809)
-- Dependencies: 2195 176 201 2249
...
--
-- TOC entry 2236 (class 2606 OID 16814)
-- Dependencies: 206 2199 203 2249
...
--
-- TOC entry 2234 (class 2606 OID 16819)
-- Dependencies: 204 203 2199 2249
-- Name: fk_usuario; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- TOC entry 2224 (class 2606 OID 16824)
-- Dependencies: 201 2195 190 2249
...
--
-- TOC entry 2225 (class 2606 OID 16829)
-- Dependencies: 190 2199 203 2249
...
--
-- TOC entry 2216 (class 2606 OID 16834)
-- Dependencies: 2162 178 178 2249
...
--
-- TOC entry 2217 (class 2606 OID 16839)
-- Dependencies: 2199 178 203 2249
...
--
-- TOC entry 2218 (class 2606 OID 16844)
-- Dependencies: 182 178 2166 2249
...
--
-- TOC entry 2219 (class 2606 OID 16849)
-- Dependencies: 2181 182 192 2249
...
--
-- TOC entry 2220 (class 2606 OID 16854)
-- Dependencies: 184 2164 180 2249
...
--
-- TOC entry 2221 (class 2606 OID 16859)
-- Dependencies: 184 2185 196 2249
...
--
-- TOC entry 2222 (class 2606 OID 16864)
-- Dependencies: 2199 203 184 2249
...
--
-- TOC entry 2227 (class 2606 OID 16869)
-- Dependencies: 199 196 2189 2249
...
--
-- TOC entry 2228 (class 2606 OID 16874)
-- Dependencies: 203 196 2199 2249
...
--
-- TOC entry 2223 (class 2606 OID 16879)
-- Dependencies: 188 203 2199 2249
...
-- Completed on 2013-11-07 15:09:09 CST
